...
#[get("/get")]
async fn get_config() -> Json<ConfigJson> {
...
fn update_config(config: Json<ConfigJson>) -> &'static str {
    unsafe {
        DEVICE.update_peer_ip(IpAddr::from_str(&*config.peer_ip).unwrap());
        DEVICE.update_server_socket(config.server_socket.clone());
        DEVICE.update_server_pubkey(config.peer_pubkey.clone());
        DEVICE.update_peer_ssid(config.peer_ssid.clone());
        DEVICE.update_peer_passwd(config.peer_passwd.clone());
        DEVICE.update_new_keypair(config.peer_private_key.clone(), config.peer_pubkey.clone())
    }
    "200"
}

#[post("/reload")]
async fn reload_config() -> &'static str {
    unsafe { DEVICE.overwrite_config() }
}
...
#[post("/gen")]
async fn gen_keypair() -> Json<KeyPairConfig> {
    let keypair;
    unsafe {
    ...
    match std::fs::read_to_string("./config.json") {
        Ok(json) => {
            ...
            unsafe {
                ...
                DEVICE.update_server_socket(symbol_parse(data["server_socket"].to_string()));
                DEVICE.update_server_pubkey(symbol_parse(data["server_pubkey"].to_string()));
                DEVICE.update_new_keypair(
                    symbol_parse(data["peer_private_key"].to_string()),
                    symbol_parse(data["peer_pubkey"].to_string()),
                );
                DEVICE.update_peer_ssid(symbol_parse(data["peer_ssid"].to_string()));
                DEVICE.update_peer_passwd(symbol_parse(data["peer_passwd"].to_string()));
                DEVICE.start();
            }
        }
        Err(e) => {
